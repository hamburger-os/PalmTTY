...
for (const [directory, name] of requiredSkills) {
  ...
  if (!frontmatter || !new RegExp(`^name:\\s*${name}import { readdir, readFile, stat } from "node:fs/promises";
import path from "node:path";
...
, "m").test(frontmatter[1]) || !/^description:/m.test(frontmatter[1])) {
    failures.push(`.agents/skills/${directory}/SKILL.md is missing required Agent Skills frontmatter`);
  }
}
