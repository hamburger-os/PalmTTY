...
On touch devices, a one-finger vertical drag that starts inside the terminal surface belongs to terminal scrollback, not to the surrounding page. The terminal stack version is governed by `terminal-stack.json`. PalmTTY keeps one narrow host-local fallback in `terminal-touch-scroll.ts`: it may own a one-finger gesture only while xterm is on the normal buffer and `mouseTrackingMode === "none"`, then scroll through xterm's public `scrollLines` API and cancel browser page panning. Alternate-buffer or active mouse-protocol gestures remain xterm-owned. Do not add a second DOM scroll viewport, a document-level touch handler, or global gesture interception. The terminal host itself owns browser touch panning via `touch-action` plus non-passive local touch cancellation, so terminal scroll does not depend on changing the surrounding workbench geometry.
