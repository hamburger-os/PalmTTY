import {
  createContext,
  useContext,
  useEffect,
  useMemo,
  useState,
  type ReactNode
} from "react";

export type Locale = "en" | "zh-CN";

const en = {
    "app.eyebrow": "self-hosted remote dev",
    "app.language": "Language",
    "appearance.theme": "Theme",
    "appearance.theme.spectrum": "Spectrum",
    "appearance.theme.obsidian": "Obsidian",
    "appearance.theme.frosted": "Frosted",
    "appearance.performance": "Visual effects",
    "appearance.performance.quality": "Effects",
    "appearance.performance.performance": "Battery saver",
    "appearance.reducedMotion": "The system reduced-motion preference is active. Decorative motion is disabled.",
    "appearance.reducedMotionShort": "Motion reduced",
    "common.cancel": "Cancel",
    "common.confirm": "Confirm",
    "auth.connecting": "Connecting…",
    "auth.description": "Enter the access token configured on your workstation.",
    "auth.token": "Access token",
    "auth.signIn": "Sign in",
    "auth.signingIn": "Signing in…",
    "auth.signOut": "Sign out",
    "workspaces.title": "Workspaces",
    "workspaces.add": "Add workspace",
    "workspaces.empty": "No workspaces yet. Add one from this page to start a terminal.",
    "workspaces.newSession": "New session",
    "workspaces.edit": "Edit",
    "workspaces.host": "Host",
    "workspaces.wsl": "WSL",
    "workspaces.defaultShell": "default shell",
    "workspaces.deleteConfirm": "Delete this workspace? Existing exited sessions are not affected; workspaces with an active session cannot be deleted.",
    "sessions.title": "Sessions",
    "sessions.refresh": "Refresh",
    "sessions.empty": "No sessions yet.",
    "sessions.terminate": "Terminate",
    "sessions.terminating": "Terminating…",
    "sessions.terminateTitle": "Terminate session?",
    "sessions.terminateConfirm": "Terminate this running session? This ends the shell and any foreground CLI in it and affects {count} current connection(s).",
    "sessions.clear": "Clear",
    "sessions.clearing": "Clearing…",
    "sessions.clearTitle": "Clear retained session?",
    "sessions.clearConfirm": "Clear this retained session and its terminal history now?",
    "sessions.exitCode": "exit code {code}",
    "sessions.connectionOne": "{count} connection",
    "sessions.connectionOther": "{count} connections",
    "sessions.state.starting": "starting",
    "sessions.state.running": "running",
    "sessions.state.stopping": "stopping",
    "sessions.state.exited": "exited",
    "sessions.state.failed": "failed",
    "workspace.addTitle": "Add workspace",
    "workspace.editTitle": "Edit workspace",
    "workspace.name": "Name",
    "workspace.namePlaceholder": "My project",
    "workspace.cwd": "Working directory",
    "workspace.cwdHostPlaceholder": "C:\\Code\\project or /home/me/project",
    "workspace.cwdWslPlaceholder": "/home/me/project",
    "workspace.cwdBrowseHelp": "Browse directories on the PalmTTY Agent host or selected WSL distribution, not on this browser device.",
    "workspace.directoryBrowse": "Browse",
    "workspace.directoryHide": "Hide browser",
    "workspace.directoryPicker": "Choose a directory",
    "workspace.directoryPickerHelp": "Open a folder to navigate into it, then choose the current folder.",
    "workspace.directoryClose": "Close",
    "workspace.directoryLocations": "Locations",
    "workspace.directoryUp": "Up",
    "workspace.directoryRefresh": "Refresh",
    "workspace.directoryEmpty": "No child directories here.",
    "workspace.directoryTruncated": "Only the first 512 directories are shown.",
    "workspace.directoryUseCurrent": "Use this directory",
    "workspace.directoryLoading": "Loading directories…",
    "workspace.directoryFallback": "The typed path was unavailable, so the browser opened at the default location.",
    "workspace.terminalProfile": "Terminal environment",
    "workspace.terminalProfileHelp": "Choose a detected terminal profile. Installed WSL distributions appear directly here and scanning does not start them.",
    "workspace.profileDetecting": "Scanning…",
    "workspace.profileRefresh": "Rescan",
    "workspace.profileRecommended": "recommended",
    "workspace.profileCustom": "Custom…",
    "workspace.profileDetectionFailed": "Terminal profile discovery failed. Custom settings are still available.",
    "workspace.profileHostGroup": "Host shells",
    "workspace.profileWslGroup": "WSL distributions",
    "workspace.customRuntime": "Advanced runtime",
    "workspace.customRuntimeHelp": "Only use this when a detected profile does not fit. Runtime details stay hidden in the normal workflow.",
    "workspace.shellExecutable": "Shell executable",
    "workspace.shellOptional": "Optional; leave blank to use the runtime default",
    "workspace.shellArgs": "Shell arguments",
    "workspace.shellArgsHelp": "Optional; one argument per line. Arguments are passed directly to the shell executable.",
    "workspace.shellArgsPlaceholder": "-l",
    "workspace.shellHostWindows": "pwsh.exe or another executable",
    "workspace.shellHostUnix": "$SHELL, /bin/bash, /bin/zsh, …",
    "workspace.shellWsl": "/bin/bash, /usr/bin/zsh, …",
    "workspace.environment": "Environment",
    "workspace.environmentHelp": "Optional; one NAME=value entry per line. Balanced outer single/double quotes are accepted and removed, so HTTP_PROXY=\"http://127.0.0.1:10808\" is stored as the URL without quote characters. Values are applied before the shell starts. This is local workspace configuration, not a secret vault.",
    "workspace.environmentMissingEquals": "Environment line {line} must use NAME=value.",
    "workspace.environmentInvalidName": "Environment line {line} has an invalid variable name.",
    "workspace.environmentDuplicate": "Environment line {line} duplicates a variable name.",
    "workspace.environmentReserved": "Environment line {line} uses TERM, which PalmTTY manages.",
    "workspace.environmentUnbalancedQuotes": "Environment line {line} has unmatched outer quotes.",
    "workspace.environmentTooMany": "A workspace can define at most 64 environment variables.",
    "workspace.environmentValueTooLong": "Environment line {line} is too long.",
    "workspace.distribution": "WSL distribution",
    "workspace.distributionOptional": "Optional; leave blank to use the default WSL distribution",
    "workspace.distributionPlaceholder": "Ubuntu-24.04",
    "workspace.cwdWslRequired": "Choose a Linux working directory for this WSL profile. Host paths are not reused across runtimes.",
    "workspace.startupCommand": "Startup command",
    "workspace.startupOptional": "Optional; each line is sent to the shell after the terminal starts. Use Environment above for variables that must exist before the shell launches.",
    "workspace.startupPlaceholder": "codex",
    "workspace.agentPresets": "Agent presets",
    "workspace.clear": "Clear",
    "workspace.cancel": "Cancel",
    "workspace.save": "Save",
    "workspace.saving": "Saving…",
    "workspace.delete": "Delete",
    "workspace.deleteTitle": "Delete workspace?",
    "workspace.deleting": "Deleting…",
    "workbench.views": "Session views",
    "workbench.session": "Session",
    "workbench.terminal": "Terminal",
    "workbench.git": "Git",
    "workbench.files": "Files",
    "workbench.artifacts": "Artifacts",
    "terminal.back": "Sessions",
    "terminal.restart": "Restart terminal",
    "terminal.restarting": "Restarting…",
    "terminal.restartTitle": "Restart this terminal?",
    "terminal.restartConfirm": "The current PTY and its retained terminal history will be replaced. The new terminal uses the latest workspace settings and a fresh host environment.",
    "terminal.specialKeys": "Terminal special keys",
    "terminal.moreSpecialKeys": "More terminal special keys",
    "terminal.keyboard": "Keyboard",
    "terminal.moreKeys": "More",
    "terminal.lessKeys": "Less",
    "terminal.keyEscape": "Escape",
    "terminal.keyTab": "Tab",
    "terminal.keyEnter": "Enter",
    "terminal.keyArrowUp": "Arrow up",
    "terminal.keyArrowDown": "Arrow down",
    "terminal.keyArrowLeft": "Arrow left",
    "terminal.keyArrowRight": "Arrow right",
    "terminal.keyPageUp": "Page up",
    "terminal.keyPageDown": "Page down",
    "terminal.keyHome": "Home",
    "terminal.keyEnd": "End",
    "terminal.keyBackspace": "Backspace",
    "terminal.keyDelete": "Delete",
    "terminal.fontSmaller": "Smaller terminal text",
    "terminal.fontLarger": "Larger terminal text",
    "terminal.longInput": "Text",
    "terminal.longInputTitle": "Send long text",
    "terminal.longInputHelp": "Use this for a long command, pasted block, voice input, or an AI prompt. Paste preserves the terminal's bracketed-paste mode; use Paste + Enter only when you also want to submit it.",
    "terminal.composer": "Compose a long command or AI prompt…",
    "terminal.paste": "Paste",
    "terminal.pasteAndEnter": "Paste + Enter",
    "terminal.connection.connecting": "connecting",
    "terminal.connection.connected": "connected",
    "terminal.connection.reconnecting": "reconnecting",
    "terminal.connection.stopping": "stopping",
    "terminal.connection.closed": "closed",
    "terminal.sessionExited": "[PalmTTY] session exited{code}",
    "files.title": "Files",
    "files.root": "Workspace root",
    "files.up": "Up",
    "files.refresh": "Refresh",
    "files.loading": "Loading files…",
    "files.reading": "Reading file…",
    "files.empty": "This directory is empty.",
    "files.select": "Select a file to preview it.",
    "files.binary": "This binary file type is not rendered in the browser.",
    "files.imageLoading": "Loading image preview…",
    "files.truncated": "Only the first 512 entries are shown.",
    "files.previewTruncated": "Preview truncated at 512 KiB. Copy and Share still use the complete file.",
    "files.back": "Files",
    "files.copy": "Copy content",
    "files.copying": "Copying…",
    "files.copied": "Complete file copied.",
    "files.copyFailed": "Clipboard access failed. You can still select the preview text manually.",
    "files.share": "Share file",
    "files.download": "Download file",
    "files.shareFailed": "The file could not be shared or downloaded.",
    "files.exporting": "Preparing file…",
    "files.history": "History",
    "files.parentPath": "Folder: {path}",
    "artifacts.title": "Artifacts",
    "artifacts.sessionScope": "Session attachments",
    "artifacts.upload": "Add image",
    "artifacts.uploading": "Uploading…",
    "artifacts.refresh": "Refresh",
    "artifacts.help": "Images are stored privately with this Session, outside the Git workspace. Insert Path writes only the file path into the terminal; it never runs a command.",
    "artifacts.loading": "Loading attachments…",
    "artifacts.empty": "No image attachments in this Session yet.",
    "artifacts.select": "Select an image to preview it.",
    "artifacts.previewing": "Loading image…",
    "artifacts.back": "Artifacts",
    "artifacts.insertPath": "Insert path",
    "artifacts.delete": "Delete",
    "artifacts.deleteTitle": "Delete attachment?",
    "artifacts.deleteConfirm": "Delete {name} from this Session? This does not modify the project workspace.",
    "git.title": "Source Control",
    "git.refresh": "Refresh",
    "git.viewChanges": "Changes",
    "git.viewHistory": "History",
    "git.loading": "Reading Git status…",
    "git.notRepository": "This workspace is not inside a Git repository.",
    "git.clean": "Working tree clean.",
    "git.detached": "detached HEAD",
    "git.noUpstream": "no upstream",
    "git.repositoryScope": "Repository scope includes the parent repository; workspace path: {path}",
    "git.enableWrites": "Enable Git writes",
    "git.enableWritesTitle": "Enable repository write actions?",
    "git.enableWritesConfirm": "Git write actions can run repository-defined filters during staging, checkout, stash or pull. PalmTTY disables Git hooks and interactive prompts, but this repository should still be trusted before enabling writes for this view.",
    "git.writesEnabled": "Writes enabled",
    "git.fetch": "Fetch",
    "git.pull": "Pull",
    "git.push": "Push",
    "git.conflicts": "Conflicts",
    "git.staged": "Staged Changes",
    "git.changes": "Changes",
    "git.stage": "Stage",
    "git.unstage": "Unstage",
    "git.stageAll": "Stage all",
    "git.unstageAll": "Unstage all",
    "git.select": "Select a changed file to inspect its diff.",
    "git.back": "Changes",
    "git.stagedBadge": "staged",
    "git.workingTreeBadge": "working tree",
    "git.diffLoading": "Loading diff…",
    "git.noDiff": "No textual diff is available for this change.",
    "git.binaryDiff": "Binary changes are not rendered in the browser.",
    "git.restore": "Discard",
    "git.restoreTitle": "Discard working-tree changes?",
    "git.restoreConfirm": "Discard the current unstaged changes in {path}? This cannot be undone by PalmTTY.",
    "git.commitPlaceholder": "Commit message",
    "git.commit": "Commit staged changes",
    "git.branches": "Branches",
    "git.switchBranch": "Switch",
    "git.newBranch": "New branch name",
    "git.createBranch": "Create",
    "git.stash": "Stash",
    "git.stashPush": "Stash changes",
    "git.stashPop": "Pop stash",
    "git.recentCommits": "Commit history",
    "git.noCommits": "No commits yet.",
    "git.historyLoading": "Loading commit history…",
    "git.historyPath": "History for {path}",
    "git.historyAll": "All history",
    "git.historyLoadMore": "Load more",
    "git.historyLoadingMore": "Loading…",
    "git.selectCommit": "Select a commit to inspect its changed files.",
    "git.commitBack": "History",
    "git.commitFiles": "Changed files",
    "git.commitNoFiles": "No changed files are available for this commit.",
    "git.commitBody": "Commit message",
    "git.commitDiffBack": "Commit",
    "git.commitDiffLoading": "Loading commit diff…",
    "git.commitTruncated": "Commit details were truncated at the safe display limit.",
    "git.commitDiffTruncated": "Commit diff truncated at 1 MiB.",
    "git.truncated": "The Git status is incomplete. Git write and remote actions are disabled until it fits the safe bounds.",
    "git.diffTruncated": "Diff truncated at 1 MiB; destructive discard is disabled.",
    "errors.startup_failed": "PalmTTY could not finish startup.",
    "errors.runtime_capabilities_failed": "Runtime capability detection failed. Host workspaces are still available.",
    "errors.login_failed": "Sign-in failed.",
    "errors.session_create_failed": "Could not create the terminal session.",
    "errors.session_restart_failed": "The terminal could not be restarted.",
    "errors.session_action_failed": "The session action could not be completed.",
    "errors.session_not_stopped": "Stop the session before clearing it.",
    "errors.authentication_required": "Please sign in again.",
    "errors.invalid_credentials": "The access token is incorrect.",
    "errors.too_many_attempts": "Too many sign-in attempts. Try again shortly.",
    "errors.untrusted_origin": "This browser origin is not trusted by the PalmTTY Agent.",
    "errors.too_many_session_requests": "Too many session requests. Try again shortly.",
    "errors.invalid_workspace_request": "The workspace definition is invalid.",
    "errors.workspace_invalid": "The workspace could not be validated on this host.",
    "errors.workspace_not_found": "The workspace no longer exists.",
    "errors.workspace_in_use": "This workspace still has an active session.",
    "errors.too_many_workspace_requests": "Too many workspace changes. Try again shortly.",
    "errors.invalid_directory_request": "The directory browse request is invalid.",
    "errors.directory_unavailable": "That directory cannot be opened from this runtime.",
    "errors.too_many_directory_requests": "Too many directory browse requests. Try again shortly.",
    "errors.invalid_terminal_profile_request": "The terminal profile request is invalid.",
    "errors.terminal_profile_detection_failed": "Terminal environments could not be discovered.",
    "errors.too_many_terminal_profile_requests": "Too many terminal profile scans. Try again shortly.",
    "errors.invalid_workspace_file_request": "The workspace file request is invalid.",
    "errors.workspace_file_unavailable": "That workspace file or directory cannot be opened.",
    "errors.workspace_file_content_unavailable": "The complete file cannot be exported. It may exceed the 8 MiB limit or have changed while being read.",
    "errors.workspace_image_unavailable": "That file is not a supported image or cannot be previewed safely.",
    "errors.invalid_artifact_request": "The attachment request is invalid.",
    "errors.artifact_upload_failed": "The image could not be added to this Session.",
    "errors.artifact_not_found": "That Session attachment no longer exists.",
    "errors.session_not_running": "New images can be added only while the terminal Session is running.",
    "errors.session_runtime_unavailable": "This Session is missing its launch-runtime metadata. Restart the terminal and report the inconsistency if it persists.",
    "errors.too_many_artifact_requests": "Too many attachment requests. Try again shortly.",
    "errors.too_many_artifact_uploads": "Too many attachment uploads. Try again shortly.",
    "errors.artifact_too_large": "The image exceeds the 8 MiB per-file limit.",
    "errors.artifact_list_failed": "Session attachments could not be loaded.",
    "errors.artifact_preview_failed": "The attachment preview could not be loaded.",
    "errors.artifact_delete_failed": "The attachment could not be deleted.",
    "errors.invalid_git_request": "The Git request is invalid.",
    "errors.git_unavailable": "Git information is unavailable for this workspace.",
    "errors.git_state_changed": "The repository changed. PalmTTY refreshed the current state; review it and try again.",
    "errors.git_operation_failed": "The Git operation could not be completed.",
    "errors.git_remote_failed": "The Git remote operation failed. Check credentials, upstream configuration, and network access.",
    "errors.too_many_git_mutations": "Too many Git write operations. Try again shortly.",
    "errors.too_many_git_remote_requests": "Too many Git remote operations. Try again shortly.",
    "errors.too_many_workspace_tool_requests": "Too many workspace tool requests. Try again shortly.",
    "errors.http_401": "Please sign in again.",
    "errors.http_403": "This request was rejected by the security policy.",
    "errors.http_404": "The requested resource was not found.",
    "errors.http_409": "The requested change conflicts with the current state.",
    "errors.http_413": "The request payload is too large.",
    "errors.http_429": "Too many requests. Try again shortly.",
    "errors.http_500": "The PalmTTY Agent reported an internal error."
} as const;

type MessageKey = keyof typeof en;

const zhCN: Record<MessageKey, string> = {
    "app.eyebrow": "自托管远程开发终端",
    "app.language": "语言",
    "appearance.theme": "主题",
    "appearance.theme.spectrum": "炫彩流光",
    "appearance.theme.obsidian": "黑曜石",
    "appearance.theme.frosted": "白霜",
    "appearance.performance": "视觉效果",
    "appearance.performance.quality": "效果优先",
    "appearance.performance.performance": "性能优先",
    "appearance.reducedMotion": "系统已启用减少动态效果，装饰动画已停止。",
    "appearance.reducedMotionShort": "已减少动态",
    "common.cancel": "取消",
    "common.confirm": "确认",
    "auth.connecting": "正在连接…",
    "auth.description": "请输入工作站上配置的访问令牌。",
    "auth.token": "访问令牌",
    "auth.signIn": "登录",
    "auth.signingIn": "正在登录…",
    "auth.signOut": "退出登录",
    "workspaces.title": "工作区",
    "workspaces.add": "新建工作区",
    "workspaces.empty": "还没有工作区。可直接在网页端新建，然后启动终端。",
    "workspaces.newSession": "新建会话",
    "workspaces.edit": "编辑",
    "workspaces.host": "宿主机",
    "workspaces.wsl": "WSL",
    "workspaces.defaultShell": "默认 Shell",
    "workspaces.deleteConfirm": "确定删除这个工作区吗？已经退出并保留的会话不会受影响；仍有活动会话的工作区不能删除。",
    "sessions.title": "会话",
    "sessions.refresh": "刷新",
    "sessions.empty": "还没有会话。",
    "sessions.terminate": "终止",
    "sessions.terminating": "终止中…",
    "sessions.terminateTitle": "终止会话？",
    "sessions.terminateConfirm": "确定终止这个正在运行的会话吗？这会结束其中的 Shell 和前台 CLI，并影响当前 {count} 个连接。",
    "sessions.clear": "清除",
    "sessions.clearing": "清除中…",
    "sessions.clearTitle": "清除已保留会话？",
    "sessions.clearConfirm": "确定立即清除这个已保留会话及其终端历史吗？",
    "sessions.exitCode": "退出码 {code}",
    "sessions.connectionOne": "{count} 个连接",
    "sessions.connectionOther": "{count} 个连接",
    "sessions.state.starting": "启动中",
    "sessions.state.running": "运行中",
    "sessions.state.stopping": "终止中",
    "sessions.state.exited": "已退出",
    "sessions.state.failed": "失败",
    "workspace.addTitle": "新建工作区",
    "workspace.editTitle": "编辑工作区",
    "workspace.name": "名称",
    "workspace.namePlaceholder": "我的项目",
    "workspace.cwd": "工作目录",
    "workspace.cwdHostPlaceholder": "C:\\Code\\project 或 /home/me/project",
    "workspace.cwdWslPlaceholder": "/home/me/project",
    "workspace.cwdBrowseHelp": "这里浏览的是 PalmTTY Agent 宿主机或所选 WSL 发行版的目录，不是当前手机/浏览器设备的目录。",
    "workspace.directoryBrowse": "选择目录",
    "workspace.directoryHide": "收起目录",
    "workspace.directoryPicker": "选择工作目录",
    "workspace.directoryPickerHelp": "点击子目录继续进入，确认后选择当前目录。",
    "workspace.directoryClose": "关闭",
    "workspace.directoryLocations": "常用位置",
    "workspace.directoryUp": "上一级",
    "workspace.directoryRefresh": "刷新",
    "workspace.directoryEmpty": "当前目录没有可进入的子目录。",
    "workspace.directoryTruncated": "目录过多，仅显示前 512 个。",
    "workspace.directoryUseCurrent": "选择当前目录",
    "workspace.directoryLoading": "正在读取目录…",
    "workspace.directoryFallback": "当前填写的路径不可用，已打开默认位置。",
    "workspace.terminalProfile": "终端环境",
    "workspace.terminalProfileHelp": "直接选择已检测到的终端配置；已安装的 WSL 发行版会作为一级配置出现，扫描列表不会启动发行版。",
    "workspace.profileDetecting": "正在扫描…",
    "workspace.profileRefresh": "重新扫描",
    "workspace.profileRecommended": "推荐",
    "workspace.profileCustom": "自定义…",
    "workspace.profileDetectionFailed": "终端环境扫描失败；仍可使用自定义高级设置。",
    "workspace.profileHostGroup": "宿主机 Shell",
    "workspace.profileWslGroup": "WSL 发行版",
    "workspace.customRuntime": "高级运行类型",
    "workspace.customRuntimeHelp": "只有检测到的终端配置不满足需求时才需要这里；正常流程不暴露 Host/WSL 内部运行细节。",
    "workspace.shellExecutable": "Shell 可执行文件",
    "workspace.shellOptional": "可选；留空使用该运行环境的默认 Shell",
    "workspace.shellArgs": "Shell 参数",
    "workspace.shellArgsHelp": "可选；每行填写一个参数，并会作为独立 argv 直接传给 Shell。",
    "workspace.shellArgsPlaceholder": "-l",
    "workspace.shellHostWindows": "pwsh.exe 或其他可执行文件",
    "workspace.shellHostUnix": "$SHELL、/bin/bash、/bin/zsh 等",
    "workspace.shellWsl": "/bin/bash、/usr/bin/zsh 等",
    "workspace.environment": "环境变量",
    "workspace.environmentHelp": "可选；每行一个 NAME=value。允许最外层成对的单引号/双引号并会自动去除，因此 HTTP_PROXY=\"http://127.0.0.1:10808\" 会按不带引号的 URL 保存。变量在 Shell 启动前写入；这里是本机工作区配置，不是密钥保险箱。",
    "workspace.environmentMissingEquals": "环境变量第 {line} 行必须使用 NAME=value。",
    "workspace.environmentInvalidName": "环境变量第 {line} 行的变量名无效。",
    "workspace.environmentDuplicate": "环境变量第 {line} 行与已有变量名重复。",
    "workspace.environmentReserved": "环境变量第 {line} 行使用了由 PalmTTY 管理的 TERM。",
    "workspace.environmentUnbalancedQuotes": "环境变量第 {line} 行的最外层引号不成对。",
    "workspace.environmentTooMany": "每个工作区最多定义 64 个环境变量。",
    "workspace.environmentValueTooLong": "环境变量第 {line} 行内容过长。",
    "workspace.distribution": "WSL 发行版",
    "workspace.distributionOptional": "可选；留空使用默认 WSL 发行版",
    "workspace.distributionPlaceholder": "Ubuntu-24.04",
    "workspace.cwdWslRequired": "请选择这个 WSL 配置中的 Linux 工作目录；Host 路径不会跨运行环境复用。",
    "workspace.startupCommand": "启动命令",
    "workspace.startupOptional": "可选；终端启动后会逐行发送给 Shell。需要在 Shell 启动前就生效的变量，请使用上面的“环境变量”。",
    "workspace.startupPlaceholder": "codex",
    "workspace.agentPresets": "常用 Agent",
    "workspace.clear": "清空",
    "workspace.cancel": "取消",
    "workspace.save": "保存",
    "workspace.saving": "正在保存…",
    "workspace.delete": "删除",
    "workspace.deleteTitle": "删除工作区？",
    "workspace.deleting": "正在删除…",
    "workbench.views": "会话视图",
    "workbench.session": "会话",
    "workbench.terminal": "终端",
    "workbench.git": "Git",
    "workbench.files": "文件",
    "workbench.artifacts": "附件",
    "terminal.back": "会话",
    "terminal.restart": "重启终端",
    "terminal.restarting": "正在重启…",
    "terminal.restartTitle": "重启这个终端？",
    "terminal.restartConfirm": "当前 PTY 和保留的终端历史会被替换；新终端会使用最新工作区设置，并重新读取宿主机环境变量。",
    "terminal.specialKeys": "终端特殊按键",
    "terminal.moreSpecialKeys": "更多终端特殊按键",
    "terminal.keyboard": "键盘",
    "terminal.moreKeys": "更多",
    "terminal.lessKeys": "收起",
    "terminal.keyEscape": "退出键",
    "terminal.keyTab": "制表键",
    "terminal.keyEnter": "回车键",
    "terminal.keyArrowUp": "向上方向键",
    "terminal.keyArrowDown": "向下方向键",
    "terminal.keyArrowLeft": "向左方向键",
    "terminal.keyArrowRight": "向右方向键",
    "terminal.keyPageUp": "向上翻页",
    "terminal.keyPageDown": "向下翻页",
    "terminal.keyHome": "行首键",
    "terminal.keyEnd": "行尾键",
    "terminal.keyBackspace": "退格键",
    "terminal.keyDelete": "删除键",
    "terminal.fontSmaller": "缩小终端文字",
    "terminal.fontLarger": "放大终端文字",
    "terminal.longInput": "长文本",
    "terminal.longInputTitle": "发送长文本",
    "terminal.longInputHelp": "适合输入长命令、粘贴多行内容、语音输入或 AI 提示词。“粘贴”会保留终端的 bracketed-paste 语义；只有需要同时提交时才使用“粘贴并回车”。",
    "terminal.composer": "输入较长命令或 AI 提示词…",
    "terminal.paste": "粘贴",
    "terminal.pasteAndEnter": "粘贴并回车",
    "terminal.connection.connecting": "正在连接",
    "terminal.connection.connected": "已连接",
    "terminal.connection.reconnecting": "正在重连",
    "terminal.connection.stopping": "正在终止",
    "terminal.connection.closed": "已断开",
    "terminal.sessionExited": "[PalmTTY] 会话已退出{code}",
    "files.title": "文件",
    "files.root": "工作区根目录",
    "files.up": "上一级",
    "files.refresh": "刷新",
    "files.loading": "正在读取文件…",
    "files.reading": "正在读取文件内容…",
    "files.empty": "当前目录为空。",
    "files.select": "选择一个文件即可预览。",
    "files.binary": "这种二进制文件类型不会在浏览器中渲染。",
    "files.imageLoading": "正在加载图片预览…",
    "files.truncated": "目录内容过多，仅显示前 512 项。",
    "files.previewTruncated": "文件预览已在 512 KiB 处截断；“复制内容”和“分享文件”仍会读取完整文件。",
    "files.back": "文件",
    "files.copy": "复制内容",
    "files.copying": "复制中…",
    "files.copied": "已复制完整文件。",
    "files.copyFailed": "无法写入剪贴板；仍可在预览区长按选择文本复制。",
    "files.share": "分享文件",
    "files.download": "下载文件",
    "files.shareFailed": "无法分享或下载这个文件。",
    "files.exporting": "正在准备文件…",
    "files.history": "历史",
    "files.parentPath": "目录：{path}",
    "artifacts.title": "附件",
    "artifacts.sessionScope": "当前会话附件",
    "artifacts.upload": "添加图片",
    "artifacts.uploading": "正在上传…",
    "artifacts.refresh": "刷新",
    "artifacts.help": "图片保存在当前 Session 的私有目录中，不写入 Git 工作区。“插入路径”只把文件路径写入终端，不会自动执行命令。",
    "artifacts.loading": "正在读取附件…",
    "artifacts.empty": "当前 Session 还没有图片附件。",
    "artifacts.select": "选择一张图片即可预览。",
    "artifacts.previewing": "正在加载图片…",
    "artifacts.back": "附件",
    "artifacts.insertPath": "插入路径",
    "artifacts.delete": "删除",
    "artifacts.deleteTitle": "删除附件？",
    "artifacts.deleteConfirm": "确定从当前 Session 删除 {name} 吗？这不会修改项目工作区。",
    "git.title": "源代码管理",
    "git.refresh": "刷新",
    "git.viewChanges": "更改",
    "git.viewHistory": "历史",
    "git.loading": "正在读取 Git 状态…",
    "git.notRepository": "当前工作区不在 Git 仓库中。",
    "git.clean": "工作树干净。",
    "git.detached": "游离 HEAD",
    "git.noUpstream": "未设置上游",
    "git.repositoryScope": "当前 Git 范围是完整父仓库；工作区在仓库中的路径：{path}",
    "git.enableWrites": "启用 Git 写操作",
    "git.enableWritesTitle": "启用仓库写操作？",
    "git.enableWritesConfirm": "暂存、切换分支、stash 或 pull 可能触发仓库配置的 filter。PalmTTY 会禁用 Git hooks 与交互式提示，但仍应只对你信任的仓库启用本页面写操作。",
    "git.writesEnabled": "写操作已启用",
    "git.fetch": "Fetch",
    "git.pull": "Pull",
    "git.push": "Push",
    "git.conflicts": "冲突",
    "git.staged": "已暂存的更改",
    "git.changes": "更改",
    "git.stage": "暂存",
    "git.unstage": "取消暂存",
    "git.stageAll": "全部暂存",
    "git.unstageAll": "全部取消暂存",
    "git.select": "选择一个已修改文件查看 diff。",
    "git.back": "更改",
    "git.stagedBadge": "已暂存",
    "git.workingTreeBadge": "工作树",
    "git.diffLoading": "正在读取 diff…",
    "git.noDiff": "这个修改没有可显示的文本 diff。",
    "git.binaryDiff": "二进制修改不会在浏览器中渲染。",
    "git.restore": "丢弃",
    "git.restoreTitle": "丢弃工作树修改？",
    "git.restoreConfirm": "丢弃 {path} 当前未暂存的修改？PalmTTY 无法撤销该操作。",
    "git.commitPlaceholder": "提交说明",
    "git.commit": "提交已暂存更改",
    "git.branches": "分支",
    "git.switchBranch": "切换",
    "git.newBranch": "新分支名称",
    "git.createBranch": "创建",
    "git.stash": "Stash",
    "git.stashPush": "暂存工作区",
    "git.stashPop": "恢复 Stash",
    "git.recentCommits": "提交历史",
    "git.noCommits": "还没有提交。",
    "git.historyLoading": "正在读取提交历史…",
    "git.historyPath": "{path} 的历史",
    "git.historyAll": "全部历史",
    "git.historyLoadMore": "加载更多",
    "git.historyLoadingMore": "加载中…",
    "git.selectCommit": "选择一个提交即可查看本次修改的文件。",
    "git.commitBack": "历史",
    "git.commitFiles": "修改的文件",
    "git.commitNoFiles": "这个提交没有可显示的文件改动。",
    "git.commitBody": "提交说明",
    "git.commitDiffBack": "提交",
    "git.commitDiffLoading": "正在读取提交 diff…",
    "git.commitTruncated": "提交详情已在安全显示上限处截断。",
    "git.commitDiffTruncated": "提交 diff 已在 1 MiB 处截断。",
    "git.truncated": "Git 状态不完整；在状态恢复到安全范围内之前，Git 写操作和远端操作已禁用。",
    "git.diffTruncated": "diff 已在 1 MiB 处截断；为避免误操作，已禁用丢弃修改。",
    "errors.startup_failed": "PalmTTY 启动未完成。",
    "errors.runtime_capabilities_failed": "运行环境能力检测失败；仍可创建宿主机工作区。",
    "errors.login_failed": "登录失败。",
    "errors.session_create_failed": "无法创建终端会话。",
    "errors.session_restart_failed": "无法重启终端。",
    "errors.session_action_failed": "无法完成会话操作。",
    "errors.session_not_stopped": "请先终止会话，再清除会话记录。",
    "errors.authentication_required": "请重新登录。",
    "errors.invalid_credentials": "访问令牌不正确。",
    "errors.too_many_attempts": "登录尝试过于频繁，请稍后重试。",
    "errors.untrusted_origin": "当前网页来源未被 PalmTTY Agent 信任。",
    "errors.too_many_session_requests": "会话创建请求过于频繁，请稍后重试。",
    "errors.invalid_workspace_request": "工作区定义无效。",
    "errors.workspace_invalid": "该工作区无法在当前宿主机上通过验证。",
    "errors.workspace_not_found": "该工作区已不存在。",
    "errors.workspace_in_use": "该工作区仍有活动会话。",
    "errors.too_many_workspace_requests": "工作区修改过于频繁，请稍后重试。",
    "errors.invalid_directory_request": "目录浏览请求无效。",
    "errors.directory_unavailable": "当前运行环境无法打开这个目录。",
    "errors.too_many_directory_requests": "目录浏览请求过于频繁，请稍后重试。",
    "errors.invalid_terminal_profile_request": "终端环境扫描请求无效。",
    "errors.terminal_profile_detection_failed": "无法检测当前系统中的终端环境。",
    "errors.too_many_terminal_profile_requests": "终端环境扫描请求过于频繁，请稍后重试。",
    "errors.invalid_workspace_file_request": "工作区文件请求无效。",
    "errors.workspace_file_unavailable": "无法打开该工作区文件或目录。",
    "errors.workspace_file_content_unavailable": "无法导出完整文件；文件可能超过 8 MiB 限制，或读取过程中已经发生变化。",
    "errors.workspace_image_unavailable": "该文件不是受支持的图片，或无法安全预览。",
    "errors.invalid_artifact_request": "附件请求无效。",
    "errors.artifact_upload_failed": "无法把图片添加到当前 Session。",
    "errors.artifact_not_found": "该 Session 附件已不存在。",
    "errors.session_not_running": "只有终端 Session 正在运行时才能添加新图片。",
    "errors.session_runtime_unavailable": "这个 Session 缺少创建时的运行环境元数据；请重启终端，如仍出现请记录该异常。",
    "errors.too_many_artifact_requests": "附件请求过于频繁，请稍后重试。",
    "errors.too_many_artifact_uploads": "附件上传过于频繁，请稍后重试。",
    "errors.artifact_too_large": "图片超过单文件 8 MiB 限制。",
    "errors.artifact_list_failed": "无法读取 Session 附件。",
    "errors.artifact_preview_failed": "无法加载附件预览。",
    "errors.artifact_delete_failed": "无法删除附件。",
    "errors.invalid_git_request": "Git 请求无效。",
    "errors.git_unavailable": "当前工作区的 Git 信息不可用。",
    "errors.git_state_changed": "仓库状态已经变化，PalmTTY 已刷新当前状态，请确认后重试。",
    "errors.git_operation_failed": "Git 操作未能完成。",
    "errors.git_remote_failed": "Git 远端操作失败，请检查凭据、上游配置和网络。",
    "errors.too_many_git_mutations": "Git 写操作过于频繁，请稍后重试。",
    "errors.too_many_git_remote_requests": "Git 远端操作过于频繁，请稍后重试。",
    "errors.too_many_workspace_tool_requests": "工作区工具请求过于频繁，请稍后重试。",
    "errors.http_401": "请重新登录。",
    "errors.http_403": "该请求被安全策略拒绝。",
    "errors.http_404": "未找到请求的资源。",
    "errors.http_409": "该修改与当前状态冲突。",
    "errors.http_413": "请求内容过大。",
    "errors.http_429": "请求过于频繁，请稍后重试。",
    "errors.http_500": "PalmTTY Agent 发生内部错误。"
};

const messages: Record<Locale, Record<MessageKey, string>> = {
  en,
  "zh-CN": zhCN
};
type Variables = Record<string, string | number>;

function detectLocale(): Locale {
  try {
    const stored = window.localStorage.getItem("palmtty.locale");
    if (stored === "en" || stored === "zh-CN") return stored;
  } catch {
    // Storage may be unavailable in hardened browser contexts.
  }
  return navigator.language.toLowerCase().startsWith("zh") ? "zh-CN" : "en";
}

function interpolate(template: string, variables?: Variables): string {
  if (!variables) return template;
  return template.replace(/\{([a-zA-Z0-9_]+)\}/g, (match, key: string) => {
    const value = variables[key];
    return value === undefined ? match : String(value);
  });
}

type I18nValue = {
  locale: Locale;
  setLocale(locale: Locale): void;
  t(key: MessageKey, variables?: Variables): string;
  error(code: string): string;
  connections(count: number): string;
};

const I18nContext = createContext<I18nValue | null>(null);

export function I18nProvider({ children }: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(detectLocale);

  useEffect(() => {
    document.documentElement.lang = locale;
    try {
      window.localStorage.setItem("palmtty.locale", locale);
    } catch {
      // Language selection still works for the current page.
    }
  }, [locale]);

  const value = useMemo<I18nValue>(() => {
    const t = (key: MessageKey, variables?: Variables) =>
      interpolate(messages[locale][key], variables);

    return {
      locale,
      setLocale(next) {
        setLocaleState(next);
      },
      t,
      error(code) {
        const key = `errors.${code}` as MessageKey;
        return key in messages[locale] ? t(key) : code;
      },
      connections(count) {
        return t(
          count === 1
            ? "sessions.connectionOne"
            : "sessions.connectionOther",
          { count }
        );
      }
    };
  }, [locale]);

  return (
    <I18nContext.Provider value={value}>
      {children}
    </I18nContext.Provider>
  );
}

export function useI18n(): I18nValue {
  const value = useContext(I18nContext);
  if (!value) throw new Error("useI18n must be used inside I18nProvider");
  return value;
}
