...
`pnpm doctor` validates the authentication environment, security exposure rules, workspace directories, and shell executables before the Agent starts. Shells are resolved to absolute executable paths before Worker creation, so the PTY runtime does not depend on library-specific PATH lookup.

For development, run `pnpm dev`. It runs the same preflight before Vite and the Agent are launched, so a bad token, workspace, or shell fails once with an actionable startup error instead of leaving the frontend proxy retrying a dead Agent. The example development configuration already includes the Vite origin required by the exact Origin check.
