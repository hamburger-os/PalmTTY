...
在真实设备与长期运行稳定之前，不建议投入完整浏览器 IDE、可安装插件系统、多用户协作、云中继或未经设计的 reboot persistence。当前 Workbench 提供终端核心 + 有界 typed Git Source Control + 只读 Files/图片预览 + Session 图片附件上下文，不引入 Monaco/LSP/插件运行时。
