...
- Agent 端到端集成测试通过真实 Fastify HTTP/WebSocket + SessionManager 与确定性 PTY 适配器覆盖认证、Origin、子协议、消息顺序、重连 replay/snapshot、登录过期、慢客户端切断、退出与清理；
...
当前 Ruleset 配置已经作为 AI 主维护阶段的正式方案接受：不要求人工 approval 或 Code Owner review，要求 branch up-to-date，也不配置常驻 bypass。具体取舍见 [github-governance.md](github-governance.md)。
...
自动化 CI 已通过确定性 PTY 适配器验证端到端 HTTP/WebSocket/SessionManager 生命周期，并由独立 Windows smoke test 验证真实 node-pty + PowerShell 7/ConPTY 启动、resize 调用和中文 Unicode 往返。P0 的自动协议集成门禁已补齐；仍需要真实使用场景持续验证：
