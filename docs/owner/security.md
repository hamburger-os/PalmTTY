...
- 会话工作台的 Files、Git 与 Artifacts 与目录选择器、终端 WebSocket 分离，全部要求认证 + 精确 Origin，并按读取、附件上传/读取/删除、Git mutation、Git remote 分别限流。Files API 仍只接收规范化 Workspace 相对路径，Host 使用 `realpath`/symlink containment 防止越过 Workspace 根目录，WSL 在发行版内再次解析 physical path 并做根目录前缀检查；目录最多返回 512 项，文本预览最多 512 KiB。完整文件复制/分享/下载走独立 8 MiB 有界读取，复用同一 containment 与稳定 file-handle/长度校验，响应使用 `private, no-store` 与 `nosniff`；它仍是只读能力，不允许浏览器指定目标路径。图片预览是独立有界二进制读取，只返回通过服务端签名/尺寸检查的 PNG/JPEG/WebP/GIF，不因此获得 Workspace 写权限。
...
- 登录、创建 Session、Session lifecycle mutation、目录浏览、终端 Profile 发现、Workspace Files 预览/完整文件导出与 Git 读取、Session attachment 读取/上传/删除、Git mutation 与 Git remote 分别限流；
