...
Upstream issue https://github.com/xtermjs/xterm.js/issues/5489 records that touch scrolling stopped working in browser xterm 6.0.0. Upstream PR https://github.com/xtermjs/xterm.js/pull/5563 fixes that regression by restoring touch/gesture scrolling in the viewport. xterm's npm documentation describes beta builds as the channel for verifying fixes merged to master. PalmTTY therefore pins the browser pair to exact 6.1 beta builds containing that upstream fix instead of maintaining a parallel application scroll implementation. The Worker remains on the exact 6.0/0.14 stable pair until a separate change qualifies headless serialization, replay and geometry recovery against the newer line; a browser-only interaction regression is not sufficient reason to change canonical Worker state semantics.

The current upstream browser path registers xterm's `Gesture` target on the screen element. That gesture layer tracks recent touch positions/timestamps and schedules release inertia; `MouseService` routes gesture deltas according to terminal mode, and normal scrollback delegates pixel translation to `Viewport.handleTouchScroll`. PalmTTY therefore suppresses browser-native page panning at the same `.xterm-screen` boundary with CSS `touch-action: none`, but does not stop event propagation or translate pixels to rows itself. This keeps normal-buffer momentum, alternate-buffer key translation, mouse-protocol wheel reporting and scrollbar behavior under one xterm-owned interaction model.

Upstream issue https://github.com/xtermjs/xterm.js/issues/5835 documents iOS Chinese IMEs dropping punctuation/space when keydown reports keyCode 229 even though the final hidden-textarea value becomes observable later. Its proposed fix, https://github.com/xtermjs/xterm.js/pull/5836, remains open as of 2026-09-24. Upstream issue https://github.com/xtermjs/xterm.js/issues/6067 separately records Ctrl+letter, Ctrl+Space and Escape loss when a CJK IME masks the key code as 229. PalmTTY therefore keeps the governed browser xterm pin and uses a narrow public-API compatibility boundary: pending keyCode-229 xterm data is buffered until the textarea delta is known, genuine composition cancels the fallback, confirmed physical control keys are recovered only from keydown `KeyboardEvent.code`, and recovered data re-enters through `Terminal.input` so xterm `onData` remains the one browser-to-PalmTTY terminal-data boundary. Re-review and preferably remove this compatibility path after an upstream fix lands in a qualified browser xterm build.

FitAddon derives rows/columns from the xterm element's parent dimensions and subtracts padding declared on the xterm element itself; it does not subtract decorative padding placed on that parent. PalmTTY must therefore open xterm into a padding-free inner mount and keep border/radius/padding/clipping on an outer frame. Otherwise FitAddon can overestimate available height and the frame's clipping can cut the final rendered row.
