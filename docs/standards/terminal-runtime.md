...
On touch devices, upstream xterm still owns alternate-buffer and mouse-protocol gesture semantics. Real-device PalmTTY validation showed that relying on the upstream gesture path plus CSS overscroll containment alone was not sufficient to prevent browser page panning in the normal scrollback case. PalmTTY therefore uses a deliberately narrow host-local fallback only for `normal` buffer + `mouseTrackingMode === "none"`: touch movement is converted to terminal rows through xterm's public `scrollLines` API, the local `touchmove` is non-passive and prevented, and propagation is stopped before xterm's document-level gesture listener can double-handle it. The terminal host uses `touch-action: none`, so this behavior is contained at the terminal input boundary without changing the surrounding workbench geometry. PalmTTY still does not create a second DOM scroll viewport or install a document-level application touch handler.
