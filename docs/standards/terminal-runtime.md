...
Upstream issue https://github.com/xtermjs/xterm.js/issues/5489 records that touch scrolling stopped working in browser xterm 6.0.0. Upstream PR https://github.com/xtermjs/xterm.js/pull/5563 fixes that regression by restoring touch/gesture scrolling in the viewport. xterm's npm documentation describes beta builds as the channel for verifying fixes merged to master. PalmTTY therefore pins the browser pair to exact 6.1 beta builds containing that upstream fix and keeps its application fallback intentionally narrow rather than reimplementing xterm's full gesture/mouse model. The Worker remains on the exact 6.0/0.14 stable pair until a separate change qualifies headless serialization, replay and geometry recovery against the newer line; a browser-only interaction regression is not sufficient reason to change canonical Worker state semantics.

On touch devices, upstream xterm still owns alternate-buffer and mouse-protocol gesture semantics. Real-device PalmTTY validation showed that relying on the upstream gesture path plus CSS overscroll containment alone was not sufficient to prevent browser page panning in the normal scrollback case. PalmTTY therefore uses a deliberately narrow `.xterm-screen`-local fallback only for `normal` buffer + `mouseTrackingMode === "none"`: content-area touch movement is converted to terminal rows through xterm's public `scrollLines` API, local touch events are non-passive and prevented, and propagation is stopped before xterm's document-level gesture listener can double-handle them. The terminal host uses `touch-action: none` to stop browser page panning, while xterm's custom scrollbar remains under xterm's own PointerEvent drag implementation. This keeps the behavior at the terminal input boundary without changing Workbench geometry, adding a second DOM scroll viewport, or installing a document-level application touch handler.
