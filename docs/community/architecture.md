...
Workspace 现在是 Agent 持有的独立持久化状态，通过“认证 + 精确 Origin”保护的 HTTP API 在网页端创建、编辑和删除；创建 Session 时仍只提交 workspace ID。工作区新建/修改时会先验证，创建 Worker 前再次验证。Host 运行时把 Shell 解析成绝对可执行路径；Windows 上的 WSL 运行时解析 `wsl.exe`，并把发行版、cwd、Shell 作为结构化 argv 传递，不做字符串命令拼接。Worker bootstrap 只接收规范化后的运行规格。
