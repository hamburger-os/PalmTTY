...
浏览器端交互也保持单一所有权：终端滚动物理由 xterm 独占，PalmTTY 只用普通 click/tap focus bridge（以及手机 keybar 的显式键盘按钮）激活 xterm 隐藏 textarea，同时 xterm viewport 剩余区域继承终端主题背景；Git 左侧 sidebar 是 summary、change groups、Git tools 的唯一纵向滚动容器，内部 group/list 只是内容，不再成为嵌套 scroll owner。
