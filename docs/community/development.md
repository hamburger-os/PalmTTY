...
仓库级变更可以直接执行 `pnpm check`。CI 在 Windows 和 Ubuntu 上使用 `pnpm install --frozen-lockfile` 安装依赖；Windows 测试还会真实启动 PowerShell 7/ConPTY 并验证 Unicode 往返。PR 还会执行生产依赖安全审计和 CodeQL；受保护的 `main` 必须通过 PR 和四项自动检查，但当前不强制人工 approval。修改依赖清单时必须同步更新 `pnpm-lock.yaml`。
