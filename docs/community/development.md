...
- Treat workspace CRUD as a high-trust remote mutation surface; keep authentication, exact Origin checks, persistence validation and Session-by-ID creation intact.
- Do not broaden remote environment/process authority without an architecture/security review.
...
- 优先建立小而可测试的边界。
- 不要为了调试增加终端 I/O 日志。
- Workspace CRUD 属于高信任远程修改面，必须保持认证、精确 Origin、持久化验证以及“Session 只按 ID 创建”的边界。
- 如果继续扩大远程环境变量或进程权限，必须先进行架构与安全审查。
