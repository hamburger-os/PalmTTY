...
- `lan` requires authentication and derives exact private/overlay HTTP Origins automatically. `reverseProxy` and `https` require authentication plus explicit HTTPS Origins; Secure-cookie behavior is derived from those modes rather than configured independently.
