...
PalmTTY 自启动始终属于当前用户：Windows Task Scheduler 使用交互用户 token + 最低权限，Linux 使用 `systemd --user`。Windows 任务使用系统 Windows PowerShell 的隐藏窗口模式只作为展示与生命周期包装层；它同步运行 Node Agent 并传递退出码，不改变用户 token，也不改变提权边界。PalmTTY 不安装 LocalSystem/root service，也不会静默开启 Linux linger。
