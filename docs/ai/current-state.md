...
- on touch devices, PalmTTY has a host-local normal-buffer fallback because real-device validation showed the upstream xterm gesture path plus overscroll containment still allowed page scrolling: only `normal` buffer + `mouseTrackingMode === "none"` is intercepted, movement is converted to xterm `scrollLines`, terminal-local touch cancellation prevents page panning, and alternate-buffer/mouse-protocol gestures remain xterm-owned; there is still no second DOM scroll owner or document-level touch shim
