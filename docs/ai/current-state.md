...
- Real owner workstation + mobile Safari/Chrome + long-running Codex validation remains required. CI guards the terminal dependency/version contract plus static frame/mount and xterm-gesture ownership markers, but it does not fully emulate real iOS/Android gesture physics, soft-keyboard focus, browser chrome transitions or inertial feel.
