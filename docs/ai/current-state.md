...
- authenticated + exact-Origin Git workbench integration for the repository containing the Workspace cwd: explicit repository scope, porcelain-v2 branch/upstream/ahead-behind/status, bounded textual diff including untracked previews, recent history and local branches; reads disable external diff/textconv/fsmonitor execution, and all helper subprocesses remove the reserved `PALMTTY_*` control namespace plus any separately configured login-token variable
- typed Git write APIs cover stage/unstage, stale-safe destructive restore, commit, branch create/switch, stash push/pop, and non-interactive fetch/pull/push; no browser-supplied arbitrary Git argv exists. Mutations require the current status token plus an explicit repository-code-execution acknowledgement, destructive restore additionally verifies the loaded diff snapshot, Git hooks and interactive prompts are disabled, while normal Git filters remain possible for trusted repositories
...
- Git pane is a source-control workbench: repository scope/branch/tracking state, conflict/staged/unstaged/untracked groups, structured unified diff with line numbers, untracked previews, clean-state recent history, stage/unstage/all actions, stale-safe discard, commit, branch create/switch, stash, and fetch/pull/push. Write controls stay disabled until the user acknowledges the trusted-repository filter boundary for the current view; status refreshes on entry, focus/visibility and a low-frequency visible-tab poll
