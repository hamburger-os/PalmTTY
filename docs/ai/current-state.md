...
- on touch devices, PalmTTY has an `.xterm-screen`-local normal-buffer fallback because real-device validation showed the upstream xterm gesture path plus overscroll containment still allowed page scrolling: only `normal` buffer + `mouseTrackingMode === "none"` content swipes are intercepted and converted to xterm `scrollLines`; terminal-local cancellation prevents page panning, the xterm scrollbar keeps its own PointerEvent drag path, and alternate-buffer/mouse-protocol gestures remain xterm-owned; there is still no second DOM scroll owner or document-level touch shim
...
- Real owner workstation + mobile Safari/Chrome + long-running Codex validation remains required. CI guards the terminal dependency/version contract and the fallback's row-delta math/static ownership markers, but it does not emulate real iOS/Android gesture physics, soft-keyboard focus, or inertial scrolling.
