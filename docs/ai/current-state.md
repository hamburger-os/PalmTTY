...
- Real owner workstation + mobile Safari/Chrome + long-running Codex validation remains required. CI guards the terminal dependency/version contract plus static frame/mount, click-to-focus, terminal-surface and single-owner Git-scroll markers, but it does not fully emulate real iOS/Android gesture physics, whether a particular browser version actually presents the soft keyboard after focus, browser chrome transitions or inertial feel.
