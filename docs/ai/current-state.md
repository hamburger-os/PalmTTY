...
- authenticated + exact-Origin read-only Git integration for the repository containing the Workspace cwd: bounded branch/upstream/ahead-behind/status and textual staged/working-tree diff, with external diff/textconv and fsmonitor execution disabled; login-token environment keys are removed from helper subprocesses
