...
- terminal stack policy rejects ranged/drifted `@xterm/*` dependencies and the known browser `@xterm/xterm 6.0.0` touch-scroll regression; frozen-lock installation must resolve the exact browser/Worker set from `terminal-stack.json`; the PalmTTY fallback tests pixel-to-row accumulation/direction while theme/static checks require normal-buffer/no-mouse-protocol ownership and TerminalView lifecycle wiring
...
- on mobile Safari/Chrome (real device preferred), generate more than one viewport of normal-buffer output, swipe in both directions across the terminal content and confirm terminal history moves while the page remains fixed; separately drag the xterm scrollbar thumb in both directions and confirm it remains xterm-pointer-owned, tracks in the expected direction, and never hands the gesture to page scrolling; let new output arrive while scrolled back and confirm it does not force the viewport to the bottom, then return to the live bottom; verify tap-to-focus/soft keyboard and the special-key bar still work; enter an alternate-buffer or mouse-tracking TUI and confirm PalmTTY's fallback does not steal its gesture path;
