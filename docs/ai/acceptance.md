...
- workspace Git status/diff APIs require authentication + exact Origin, handle non-repositories without failing the Agent, bound output, reject path traversal, disable external diff/textconv/fsmonitor execution, and do not inherit the reserved `PALMTTY_*` control namespace or any separately configured PalmTTY auth-token environment key
