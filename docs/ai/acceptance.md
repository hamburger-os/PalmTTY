...
- on mobile Safari/Chrome (real device preferred), generate more than one viewport of normal-buffer output, swipe in both directions across the terminal content and confirm xterm-owned movement is continuous and retains natural release inertia while the page remains fixed; return to the live bottom and verify the final text row is fully visible with no lower-half clipping; separately drag the xterm scrollbar thumb in both directions; let new output arrive while scrolled back and confirm it does not force the viewport to the bottom; verify tap-to-focus/soft keyboard and the special-key bar still work; enter an alternate-buffer or mouse-tracking TUI and confirm xterm keeps its mode-aware touch semantics; rotate the device and trigger browser/soft-keyboard viewport changes, confirming refit does not reconnect the Session or emit redundant geometry when rows/cols are unchanged;
