...
- terminal stack policy rejects ranged/drifted `@xterm/*` dependencies and the known browser `@xterm/xterm 6.0.0` touch-scroll regression; frozen-lock installation must resolve the exact browser/Worker set from `terminal-stack.json`; theme/static checks require a decorative `terminal-frame` around a padding-free `terminal-mount`, xterm opened directly into that mount, screen-local `touch-action: none`, a 16px xterm helper textarea, SessionWorkbench-only VisualViewport ownership with pinch-zoom escape, and no PalmTTY touch listener or `terminal.scrollLines` gesture shim
...
- on mobile Safari/Chrome (real device preferred), generate more than one viewport of normal-buffer output, swipe in both directions across the terminal content and confirm xterm-owned movement is continuous and retains natural release inertia while the page remains fixed; return to the live bottom and verify the final text row is fully visible with no lower-half clipping; separately drag the xterm scrollbar thumb in both directions; let new output arrive while scrolled back and confirm it does not force the viewport to the bottom; verify tap-to-focus/explicit keyboard control opens the soft keyboard, the header/terminal/key bar all remain inside the visible area above it, typed input is visible, keyboard dismissal restores the workbench, initial page load is correctly sized, and pinch zoom remains available; enter an alternate-buffer or mouse-tracking TUI and confirm xterm keeps its mode-aware touch semantics; rotate the device and trigger browser/soft-keyboard viewport changes, confirming mount-driven refit does not reconnect the Session or emit redundant geometry when rows/cols are unchanged;
