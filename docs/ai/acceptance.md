...
- on mobile Safari/Chrome (real device preferred), generate more than one viewport of normal-buffer output, swipe in both directions anywhere inside the terminal including over the scrollbar and confirm terminal history moves while the page remains fixed; let new output arrive while scrolled back and confirm it does not force the viewport to the bottom, then return to the live bottom; verify tap-to-focus/soft keyboard and the special-key bar still work; enter an alternate-buffer or mouse-tracking TUI and confirm PalmTTY's fallback does not steal its gesture path;
