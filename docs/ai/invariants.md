...
8. **Workspace mutation is explicit and persistent.** Authenticated, exact-Origin Web requests may create/edit/delete persistent workspace definitions, including cwd/runtime/shell/startup command. Session creation itself accepts only a workspace ID, and the Web workspace model must not expose arbitrary environment-variable injection. Workspace launch targets are validated on mutation and again before Worker bootstrap.
