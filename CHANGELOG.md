...
- Keep the iPhone workbench inside the real visible viewport when Safari browser chrome or the soft keyboard changes geometry: SessionWorkbench owns the current VisualViewport frame without writing browser zoom, TerminalView refits only through its mount ResizeObserver, xterm's hidden input uses a 16px mobile-safe font size, and supported browsers opt into `interactive-widget=resizes-content` as progressive enhancement.
